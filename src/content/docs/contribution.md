...
Documentation is written using [md](https://vitepress.dev/guide/markdown). You can find the documentation files in the `apps/web/src/content` directory.
...
See the [`Accordion`](https://github.com/radix-vue/shadcn-vue/tree/v0.10.2/apps/web/src/lib/registry/default/ui/accordion) source code as an example.
...
2. If there are changes, you should review thoese changes and try to apply them on `shadcn/vue` codebase and create a PR, remember to update the `latestSyncCommitTag` in [this file](https://github.com/radix-vue/shadcn-vue/blob/dev/apps/web/src/content/docs/contribution.md) too.
